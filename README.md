...
**▶ [Watch the 2:43 demo](https://youtu.be/slyluQxHFXA)** — the app running on an Android TV device, including the on-screen refusal and a live Amazon Bedrock Nova Pro description.
...
| **Extended Audio Description (WCAG 2.2 SC 1.2.7)** | **Verified** | Viewer setting (default OFF). When OFF: 42 playable, 2 refused (byte-identical to baseline). When ON: 42 normal + 2 extended descriptions delivered via safe video pause (`sintel-ad-11` at 152.05s, `sintel-ad-28` at 456.80s), 0 refused. Dynamic watchdog derived per description; assertive screen reader announcement (`accessibilityLiveRegion="assertive"`); 10-foot remote accessible toggle button |
